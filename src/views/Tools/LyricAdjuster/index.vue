<!-- 歌词调整组件 -->
<template>
  <h1 class="card-title mt-4">Lrc 歌词时间轴调整</h1>
  <div class="tools-lyric-adjuster">
    ...
    <UnderLineMsg ref="oprMsgRef"/>
  </div>
</template>
<script lang="ts" setup>
import { reactive, ref, watch } from 'vue';
import UnderLineMsg from '@/components/UnderLineMsg';
import { clickDownload, debounce } from "@/common/utils.ts";

const props = withDefaults(defineProps<{
  inputBoxHeight?: string
}>(), {
  inputBoxHeight: '300px'
});
...
let lrcStartTime = -1;
watch(lyricContent, (value) => debounce(() => initData(value)));
